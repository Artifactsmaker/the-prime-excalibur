...
let onlinePresenceRegistered = false;

function renderUsageMetrics(
  visits = readUsageCount("oiBoxVisits"),
  executions = readUsageCount("oiBoxExecutions"),
  online = document.hidden ? 0 : 1,
  scopeKey = "counterFallback",
) {
  if ($("#visitCount")) $("#visitCount").textContent = visits.toLocaleString();
  if ($("#onlineCount")) $("#onlineCount").textContent = Math.max(0, online).toLocaleString();
...
async function syncGlobalCounters(registerVisit = false, incrementExecution = false, registerOnline = false) {
...
function initializeUsageMetrics() {
  ...
  renderUsageMetrics();
  syncGlobalCounters(true, false, true);
  window.setInterval(() => {
